...
import { ArrowUpRight, ChevronDown, Calendar, Clock } from "lucide-react";
  ...
  return (
    <div className="relative h-full flex flex-col overflow-hidden">
      ...
      >
        ...
        <Link
          href="/blogs"
          className="flex items-center gap-1 text-xs text-primary group"
        >
                ...
                {isExpanded && (
                  ...
                  >
                    <div className="pb-4 space-y-3">
                      {/* Blog Card */}
                      <div className="relative rounded-xl overflow-hidden bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border border-white/10 p-4">
                        {/* Category Badge */}
                        <div className="mb-2">
                          <span className="inline-block text-[10px] px-2 py-1 rounded-full bg-primary/20 text-primary font-medium">
                            {blog.category}
                          </span>
                        </div>
                        
                        {/* Description */}
                        <p className="text-xs text-muted-foreground/90 line-clamp-6 mb-3">
                          {blog.description}
                        </p>
                        ...
                        <div className="flex flex-wrap gap-1 mt-3">
                          {blog.tags.slice(0, 3).map((tag) => (
                            <span 
                              key={tag} 
                              className="text-[10px] px-2 py-0.5 rounded-full bg-white/5 text-muted-foreground/70 border border-white/5"
                            >
                              {tag}
                            </span>
                          ))}
