    ...
    return (
      <div className={`rich-text-content prose prose-lg dark:prose-invert max-w-none ${className}`}>
        <p className='text-muted-foreground italic'>
          Content not available. Please edit this blog post to convert it to markdown format.
        </p>
      </div>
    );
  }
      ...
      <ReactMarkdown
        ...
        components={{
          ...
          pre: ({ node, children, ...props }: any) => (
            <pre
              className='bg-gray-900 dark:bg-gray-950 text-gray-100 p-4 rounded-lg overflow-x-auto my-6 relative border border-gray-800'
              {...props}
            >
              {children}
            </pre>
          ...
          blockquote: ({ node, ...props }) => (
            <blockquote
              className='border-l-4 border-blue-500 dark:border-blue-400 pl-4 py-2 my-4 italic bg-gray-50 dark:bg-gray-900 rounded-r'
              {...props}
            />
          ),
          a: ({ node, ...props }) => (
          ...
          tr: ({ node, ...props }) => (
            <tr
              className='border-b border-gray-300 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors'
              {...props}
            />
          ),
          th: ({ node, ...props }) => (
            <th
              className='border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 px-4 py-3 font-semibold text-left'
              {...props}
            />
          ),
          td: ({ node, ...props }) => (
