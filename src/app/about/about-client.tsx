            ...
            <div className='flex flex-wrap gap-1.5 sm:gap-2'>
              {skillsByCategory.frontend.map((skill) => (
                <Badge
                  key={skill.name}
                  variant='secondary'
                  className='px-3 sm:px-4 py-1.5 sm:py-2 text-sm bg-white/5 hover:bg-white/10 transition-colors'
                >
                  {skill.name}
                </Badge>
              ))}
      ...
      >
        ...
        <div className='space-y-4 sm:space-y-5 md:space-y-6'>
          {education.map((edu) => (
            <div
              key={edu.id}
              className='p-4 sm:p-5 md:p-6 rounded-2xl bg-card/50 border border-white/10'
            >
